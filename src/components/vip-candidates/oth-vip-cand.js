...
export const OthVipCandidates = () => {
  const [vipData, setVipData] = useState([]);

  useEffect(() => {
    axios
      .get("/json/madhya_pradesh/english/vipcandidates_wonlead.json")
      .then((response) => {
    ...
    >
      ...
      {vipData.map((candidate) => (
        <SwiperSlide key={candidate.cand_name}>
          <div
            className="card-background h-64 md:h-80 flex flex-col justify-between"
            key={candidate.cand_name}
          >
            <div className="p-3">
            ...
            <div className="flex items-center  bottom-0 inset-x-0 relative bg-[#ff6100]">
              ...
              <img
                className=" h-48 absolute right-0 bottom-0 swiper-lazy"
                src={images.modi}
                alt="Fourstates"
              />
            </div>
